...
*/
    ...
    public String getAlarmsHeaderDetail(@PathVariable Integer id) throws JsonProcessingException {
        ...
        if(null!=alarmsHeader){
            eventId = alarmsHeader.getEventId();
        }
        //List<AlarmsInformation> list =getAllAlarmsInformationByeventId(eventId);
        List<AlarmsInformation> list =new ArrayList<>();
        if(!"ab305d54-85b4-a31b-7db2-fb6b9e546015_s".equals(eventId)){
        list = alarmsInformationService.getAllAlarmsInformationByeventId(eventId);
        }else{
            AlarmsInformation alarmsInformation_s = new AlarmsInformation();
            alarmsInformation_s.setId(4);
        }
...
*/
    ...
    /*@RequestMapping(value = {"/alarm/getAlarmDataByStatus/{status}","/alarm/getAlarmDataByStatus/{status}/{eventName}/{sourceName}/{eventServrity}/{reportingEntityName}/{createTime}/{endTime}"},method =RequestMethod.GET,produces = "application/json")
    public String getAlarmDataByStatus(@PathVariable String status, @PathVariable(required = false) String eventName,@PathVariable(required = false) String sourceName,@PathVariable(required = false) String eventServrity,@PathVariable(required = false) String reportingEntityName,@PathVariable(required = false) String createTime,@PathVariable(required = false) String endTime) throws JsonProcessingException {
        ...
        try {
           createTime_s =(null!=createTime ? new SimpleDateFormat("yyyy-MM-dd HH:mm").parse(createTime) : null);
          endTime_s =(null!=endTime ? new SimpleDateFormat("yyyy-MM-dd HH:mm").parse(endTime) : null);
       ...
       Set<String> sourceNameList = new HashSet<>();
        ...
        Set<String> eventServrityList = new HashSet<>();
        Set<String> sourceIdList = new HashSet<>();


       List<AlarmsHeader> list = alarmsHeaderService.getAllByStatus(status,eventName,sourceName,eventServrity,reportingEntityName,createTime_s,endTime_s);
       ...
       for(int a=0;a<list.size();a++){
           ...
           eventServrityList.add(alarmsHeader.getEventServrity());
       ...
       map.put("countClose",countClose);
        ...
        map.put("eventServrityList",eventServrityList);
    ...
    /*@RequestMapping(value = {"/alarm/getAlarmListByStatus/{status}","/alarm/getAlarmListByStatus/{status}/{eventName}/{sourceName}/{eventServrity}/{reportingEntityName}/{createTime}/{endTime}"},method =RequestMethod.GET,produces = "application/json")
    public String getAlarmListByStatus(@PathVariable String status, @PathVariable(required = false) String eventName,@PathVariable(required = false) String sourceName,@PathVariable(required = false) String eventServrity,@PathVariable(required = false) String reportingEntityName,@PathVariable(required = false) String createTime,@PathVariable(required = false) String endTime) throws JsonProcessingException {
        Map map = new HashMap();
        Date createTime_s=null;
        Date endTime_s=null;
